...
def gen_apply_func(name, func_obj):
    ...
    output += "    def {}(self, inputs, name=None, outputs=None{}):\n".format(
        name[6:], str_func_kwarg(func_obj)
    )
    output += (
        "        return self.apply_op(onnx_ops.{}, inputs, name, outputs{})".format(
            name, str_pair_kwarg(func_obj)
        )
    )
    return output
...
with open(fname_oopb, "r") as old_one:
    with open(fname_oopb + ".tmp", "w") as new_one:
        ...
        print(
            "    # The following code was generated by ../update_ops.py", file=new_one
        )

        apply_fx = {
            v1: v2 for v1, v2 in onnx_ops.__dict__.items() if v1.startswith("apply_")
        }

        for v1, v2 in apply_fx.items():
            if v1.startswith(
                "apply_constant"
            ):  # skip the constant and constant_of_shape since they are overrided.
                continue
