...
class ONNXFunctionTest(unittest.TestCase):
    ...
    def test_core(self):
        ...
        self.assertTrue(np.allclose(g([2.0], [-5.0]), np.array([2.0])))
    ...
    def test_loop(self):
        ...
        self.assertEqual(loop_test(np.array([16], dtype=np.int64))[2][4], 3.0)

    @unittest.skipIf(
        pv.Version(_ort.__version__.split("-")[0]) < pv.Version("1.4.0"),
