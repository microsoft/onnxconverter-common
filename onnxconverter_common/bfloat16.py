...
def _bfloat16_to_int(list):
    '''
    Convert bfloat16 to python int.

    :param list: bfloat16 list
    :return int_list: python int list
    '''
    return [int(_) for _ in list]
...
def convert_tensor_float_to_bfloat16(tensor):
    ...
    :return tensor_bfloat16: converted TensorProto object

    Example:

    ::

        from onnxmltools.utils.bfloat16_converter import convert_tensor_float_to_bfloat16
        new_tensor = convert_tensor_float_to_bfloat16(tensor)
    ...
    if tensor.data_type == onnx_proto.TensorProto.FLOAT:
        tensor.data_type = onnx_proto.TensorProto.BFLOAT16
        # convert float_data (float type) to bfloat16 and write to int32_data
        if tensor.float_data:
            int_list = _bfloat16_to_int(bfloat16(tensor.float_data))
...
def convert_float_to_bfloat16(model):
    ...
    ::
        ...
        from onnxmltools.utils.bfloat16_converter import convert_float_to_bfloat16
        new_onnx_model = convert_float_to_bfloat16(onnx_model)
    ...
    while queue:
        ...
        for q in queue:
            ...
            if isinstance(q, onnx_proto.GraphProto):
                for n in q.node:
                    # if n is in the black list (doesn't support bfloat16), no conversion for the node,
            ...
            if isinstance(q, onnx_proto.AttributeProto):
                ...
                for n in q.graphs:
                    next_level.append(n)
                q.t.CopyFrom(convert_tensor_float_to_bfloat16(q.t))
                for n in q.tensors:
                    n = convert_tensor_float_to_bfloat16(n)
            # if q is graph, process graph.initializer(TensorProto), input, output and value_info (ValueInfoProto)
            if isinstance(q, onnx_proto.GraphProto):
                for n in q.initializer:  # TensorProto type
                    n = convert_tensor_float_to_bfloat16(n)
                # for all ValueInfoProto with tensor(float) type in input, output and value_info, convert them to
                # tensor(bfloat16) except map and seq(map). And save them in value_info_list for further processing
                for n in itertools.chain(q.input, q.output, q.value_info):
                    if n.type.tensor_type.elem_type == onnx_proto.TensorProto.FLOAT:
                        n.type.tensor_type.elem_type = onnx_proto.TensorProto.BFLOAT16
        ...
        for i in range(len(node.input)):
            ...
            for value_info in value_info_list:
                if input == value_info.name:
                    ...
                    # add Cast node (from tensor(bfloat16) to tensor(float) before current node
                    ...
                    domain_flag = 1
                    continue
        # if output's name is in the value_info_list meaning output is tensor(bfloat16) type, insert a float to
        # bfloat16 Cast node after the node, change current node's output name and create new value_info for the new name
        for i in range(len(node.output)):
            ...
            for value_info in value_info_list:
                if output == value_info.name:
                    ...
                    attrs['to'] = onnx_proto.TensorProto.BFLOAT16
