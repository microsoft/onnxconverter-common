...
from ._opt_const_folding import (
    OnnxGraphContext,
    const_folding_optimizer,
    reserve_node_for_embedded_graph,
)
...
class LinkedNode:
    ...
    def in_single_path(self):
        ...
        """
        return len(self.successor) == 1 and not self.successor[0].in_or_out and len(self.precedence) == 1

    @property
    def in_single_path_to_output(self):
        return (
            len(self.successor) == 1
            and self.successor[0].in_or_out
            and len(self.precedence) == 1
            and not self.precedence[0].in_or_out
        )
    ...
    def element_wise(self):
        return (
            ...
            else self.origin.op_type
            in ["Relu", "LeakyRelu", "PRelu", "Tanh"]
    ...
    def broadcast(self):
        return (
            False
            if self.origin is None
            else self.origin.op_type in ["Add", "And", "Div", "Equal", "Max", "Mean", "Min", "Mul", "Sub", "Sum"]
        )
    ...
    def in_single_path_and_inner(self):
        ...
        return (
            len(self.successor) == 1
            and self.successor[0] is not None
            and not self.successor[0].in_or_out
            and len(self.precedence) == 1
            and self.precedence[0] is not None
            and not self.precedence[0].in_or_out
        )
    ...
    def in_miso_and_inner(self):
        ...
        return (
            ...
            and not self.successor[0].in_or_out
            and len(self.precedence) > 1
            and self.get_precedence_by_idx(0) is not None
            and not self.get_precedence_by_idx(0).in_or_out
        )
    ...
    def generate(self):
        ...
        if not updated:
            ...
            onode.attribute.extend(attr for attr in self.origin.attribute if attr.name not in self.attributes)
            onode.attribute.extend(helper.make_attribute(attr, self.attributes[attr]) for attr in self.attributes)
...
class Solution:
    ...
    def get_perm(onode):
        ...
        try:
            return next(helper.get_attribute_value(attr) for attr in onode.attribute if attr.name == "perm")
...
class MergeSolution(Solution):
    def apply(self, node_list):
        ...
        if self.is_useless_transpose(perm_f):
            ...
            node_list = self.delete_node_1ton(node_list, self.end_p.get_precedence_by_idx(0), self.end_p, self.end)
            self.begin_n.origin = helper.make_node(
                "Transpose",
                self.begin_n.origin.input,
                self.begin_n.origin.output,
                self.begin_n.origin.name,
                perm=perm_f,
            )
        return node_list, True
...
class MergeReshapeTransposeSolution(Solution):
    ...
    def apply(self, node_list):
        ...
        reshape_initilizer = numpy_helper.from_array(
            adjust_reshape,
            name=self.begin_n.origin.name + "_initializer_" + str(MergeReshapeTransposeSolution.init_number),
        )
                ...
                if next.origin is not None:
                    if next.origin.op_type == "Reshape" and next.get_precedence_by_idx(1) is not None:
                        ...
                        if reshape_value_0[0] == 0 and cur_perm[0] == 0 and reshape_ones + 2 == len(reshape_value_0):
                            solution = MergeReshapeTransposeSolution(
                                node.get_precedence_by_idx(0),
                                node,
                                next,
                                next.successor[0],
                            )
    ...
    def find(node):
        if node.origin.op_type == "Cast" and len(node.successor) == 1:
            ...
            if next.origin is not None and next.origin.op_type == "Cast":
                ...
                if (
                    to_0 in MergeCastOptimizer.to_priority_array
                    and to_1 in MergeCastOptimizer.to_priority_array
                    and MergeCastOptimizer.to_priority_array.index(to_0)
                    > MergeCastOptimizer.to_priority_array.index(to_1)
                ):
                    solution = Solution(node.get_precedence_by_idx(0), node, next, next)
                    return solution
...
class MergeSqueezeUnsqueezeOptimizer:
    ...
    def find(node):
        if node.origin.op_type == "Squeeze" and len(node.successor) == 1:
            ...
            next = node.successor[0]
            flag = (
                next.origin is not None
                and axes_0 is not None
                and next.origin.op_type == "Unsqueeze"
                and len(next.successor) == 1
            )
            if flag:
...
def _update_broadcast_from_initializers(node, init_pred_value, cur_perm, init_idx):
    ...
    add_initilizer = numpy_helper.from_array(
        init_pred_value,
        name=node.origin.name + "_initializer_" + str(PushTransposeSolution.transpose_number),
    )
...
def _process_transpose_pad(node, node_list, node_transpose_pass_name, cur_perm_map):
    ...
    pads_value = np.asarray(
        [pads_value[reshape_perm[idx_]] for idx_ in range(len(reshape_perm))],
        dtype=np.int64,
    )
    add_initilizer = numpy_helper.from_array(
        pads_value,
...
def _process_transpose_slice(node, node_list, node_transpose_pass_name, cur_perm_map):
    cur_perm = cur_perm_map[node.get_precedence_by_idx(0).unique_name]
    add_initilizer = numpy_helper.from_array(
        np.asarray(cur_perm).astype(np.int64),
        name=node.origin.name + "_initializer_" + str(PushTransposeSolution.transpose_number),
    )
    PushTransposeSolution.transpose_number += 1
    node.initializers = [add_initilizer]
...
def _process_transpose_pass_node(node, node_list, node_transpose_pass_name, cur_perm_map):
    type_func_map = {
        "Pad": _process_transpose_pad,
        "Squeeze": _process_transpose_squeeze,
        "Unsqueeze": _process_transpose_unsqueeze,
        "Slice": _process_transpose_slice,
    }

    if node.origin.op_type in _broadcast_types:
        node_list, cur_perm_map = _process_transpose_pass_broadcast(
...
class PushTransposeSolution(Solution):
    ...
    def apply(self, node_list):
        ...
        for node_pair_ in node_transpose_pass:
            (node, prev) = node_pair_
            node_list, cur_perm_map = _process_transpose_pass_node(
                node, node_list, node_transpose_pass_name, cur_perm_map
            )
    ...
    def find(node):
        ...
        if node.in_single_path_and_inner:
            if node.origin.op_type == "Cast":
                ...
                if (
                    to_value == 1
                    and node.successor[0].in_single_path_and_inner
                    and node.successor[0].origin.op_type in _move_cast_support_types
                ):
                    solution = SwapOpSolution(
                        node.precedence[0],
                        node,
                        node.successor[0],
                    ...
                    to_value in [6, 7]
                    and node.precedence[0].in_single_path_and_inner
                    and node.precedence[0].origin.op_type in _move_cast_support_types
                ):
                    solution = SwapOpSolution(
                        node.precedence[0].precedence[0],
                        node.precedence[0],
                        node,
                        node.successor[0],
                    )
            ...
            if node.origin.op_type in _activation_node_type:
                if (
                    node.successor[0].in_single_path_and_inner
                    and node.successor[0].origin.op_type in _move_cast_support_types
                ):
                    solution = SwapOpSolution(
                        node.precedence[0],
                    ...
                    node.successor[0].in_miso_and_inner and node.successor[0].origin.op_type in _move_cast_support_types
                ):
    ...
    def is_same_node_merge(node_0, node_1, node):
        ...
        for idx_ in range(len(node_0.precedence)):
            ...
            if pred_0.unique_name == node.unique_name:
                if node_0.input[node_0.origin.input[idx_]] != node_1.input[node_1.origin.input[idx_]]:
                    return False
                continue
...
class MatmulOptimizer:
    ...
    def find(node):  # type: (LinkedNode)->Union[Solution, None]
        if (
            node.op_type == "MatMul"
            and node.get_precedence_by_idx(0).is_transpose
            and node.get_precedence_by_idx(1).is_transpose
        ):
            # also needs check the inputs of one operants is an initilizer.
            if (
                Solution.get_perm(node.get_precedence_by_idx(0)) == MatmulSolution.basic_transpose
                or Solution.get_perm(node.get_precedence_by_idx(1)) == MatmulSolution.basic_transpose
            ):
...
def _process_optimization(node_list, target_opset=None):
    optimizers = [
        ...
        TransposeOptimizer,
        RedundantOptimizer,
        MergePadConvOptimizer,
        MergeReshapeOptimizer,
        MergeCastOptimizer,
        MergePadTransposeConvOptimizer,
        MergeSqueezeUnsqueezeOptimizer,
        SwapOpOptimizer,
        MergeCommonSequenceOptimizer,
    ]
...
def optimize_onnx(onnx_nodes, nchw_inputs=None, inputs=None, outputs=None, target_opset=None):
    ...
    node_list = LinkedNode.build_from_onnx(
        onnx_nodelist,
...
def _generate_graph_from_nodelist(node_list, initializers, model_name, inputs, outputs):
    ...
    nodes = regenerated
    graph = helper.make_graph(nodes, model_name, inputs, outputs, initializers_copy)
    return graph
...
def optimize_onnx_graph(
    ...
    outputs=None,
    initializers=None,
    stop_initializers=None,
    model_value_info=None,
    model_name=None,
    target_opset=None,
):
    """
    ...
    """
    if target_opset < 9:
        raise Exception(f"target_opset = {target_opset}, Use optimize_onnx_graph for opset >= 9")
...
def optimize_onnx_model(origin_model, nchw_inputs=None, stop_initializers=None):
    ...
    opt_graph = optimize_onnx_graph(
        ...
        target_opset=next(
            opset_.version for opset_ in origin_model.opset_import if opset_.domain == "" or opset_.domain == "ai.onnx"
        ),
    )
