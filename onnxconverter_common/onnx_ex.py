...
DEFAULT_OPSET_NUMBER = (
    15  # The maximum opset supported by the converter in the code branch.
)
...
def make_model_ex(
        ...
        if op_domain == "" or op_domain == "ai.onnx":
            if target_default_opset < op_version:
                ...
                getLogger("onnxmltools").warning(
                    "The maximum opset needed by this model is only %d." % op_version
                )
            else:
                pass
    ...
    onnx_model.producer_name = (
        kwargs.get("producer_name")
        if "producer_name" in kwargs
        else utils.get_producer()
    )
    onnx_model.producer_version = utils.get_producer_version()
    onnx_model.domain = (
        kwargs.get("domain") if "domain" in kwargs else utils.get_domain()
    )
